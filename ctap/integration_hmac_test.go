...
func (m *memSaver) RetrieveData() []byte { if m.data == nil { return nil }; return append([]byte{}, m.data...) }
func (m *memSaver) Passphrase() string { return "passphrase" }

type autoApprove struct{}
func (a *autoApprove) ApproveClientAction(_ fido_client.ClientAction, _ fido_client.ClientActionRequestParams) bool { return true }
...
func parseExtensionsFromAuthData(t *testing.T, authData []byte) map[string][]byte {
    t.Helper()
    if len(authData) < 37 { t.Fatalf("authData too short: %d", len(authData)) }
    ext := make(map[string][]byte)
    if len(authData) == 37 { return ext }
    tail := authData[37:]
    var m map[string][]byte
    err := cbor.Unmarshal(tail, &m)
    if err != nil { t.Fatalf("cbor unmarshal ext: %v", err) }
    return m
}

func newClient(t *testing.T) *fido_client.DefaultFIDOClient {
    caPriv, err := identities.CreateCAPrivateKey()
    if err != nil { t.Fatalf("create CA priv: %v", err) }
    ca, err := identities.CreateSelfSignedCA(caPriv)
    if err != nil { t.Fatalf("create CA: %v", err) }
    key := sha256.Sum256([]byte("test"))
    saver := &memSaver{}
    appr := &autoApprove{}
    c := fido_client.NewDefaultClient(ca, caPriv, key, true, appr, saver)
    c.SetPIN([]byte("1234"))
    return c
}
...
func TestHmacSecretTwoSalts(t *testing.T) {
    ...
    if ctapStatusCode(respMC[0]) != ctap1ErrSuccess { t.Fatalf("MC failed: %x", respMC[0]) }
    var mc makeCredentialResponse
    if err := cbor.Unmarshal(respMC[1:], &mc); err != nil { t.Fatalf("decode MC: %v", err) }
    ...
    ad := mc.AuthData
    if len(ad) < 37 { t.Fatalf("authData too short: %d", len(ad)) }
    // Skip to attested data: flags has AT bit set, so next is attested data
    att := ad[37:]
    if len(att) < 16+2 { t.Fatalf("attested data too short: %d", len(att)) }
    // Skip AAGUID(16), read L (2)
    l := int(att[16])<<8 | int(att[17])
    if len(att) < 18+l { t.Fatalf("attested cred id too short") }
    ...
    if err != nil { t.Fatalf("marshal hmac-secret input: %v", err) }
    ...
    if ctapStatusCode(respGA[0]) != ctap1ErrSuccess { t.Fatalf("GA failed: %x", respGA[0]) }
    var ga getAssertionResponse
    if err := cbor.Unmarshal(respGA[1:], &ga); err != nil { t.Fatalf("decode GA: %v", err) }
    // Parse extensions
    exts := parseExtensionsFromAuthData(t, ga.AuthenticatorData)
    out := exts["hmac-secret"]
    if len(out) != 64 { t.Fatalf("unexpected hmac-secret len: %d", len(out)) }
    // Verify outputs
    // find credRandom of created credential
    var credRandom []byte
    for _, c := range client.Identities() {
        if bytes.Equal(c.ID, credID) { credRandom = c.CredRandom; break }
    }
    if len(credRandom) != 32 { t.Fatalf("credRandom not found or invalid length: %d", len(credRandom)) }
...
func TestClientPinGetPINToken(t *testing.T) {
    ...
    if ctapStatusCode(resp[0]) != ctap1ErrSuccess { t.Fatalf("getPINToken failed: %x", resp[0]) }
    var out clientPINResponse
    if err := cbor.Unmarshal(resp[1:], &out); err != nil { t.Fatalf("decode getPINToken resp: %v", err) }
    if len(out.PinToken) == 0 { t.Fatalf("no pinToken returned") }
